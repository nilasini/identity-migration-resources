...
public class OAuthTokenDataTransformerV5110 implements DataTransformer {
    ...
    public List<JournalEntry> transform(List<JournalEntry> journalEntryList, PipelineContext context)
        ...
        log.info("LOG PATCH: TABLE NAME: " + context.getPipelineConfiguration().getTableName() +
                " ITERATION: " + context.getProperty("iteration") + " Transform journal entry with " +
                "OAuthTokenDataTransformerV5110.");
        try {
            ...
            for (JournalEntry entry : journalEntryList) {
                ...
                if (accessToken != null) {
                    log.info("LOG PATCH: TABLE NAME: " + context.getPipelineConfiguration().getTableName() +
                            " ITERATION: " + context.getProperty("iteration") + " retrieving access token.");
                }
                String refreshToken = getObjectValueFromEntry(entry, COLUMN_REFRESH_TOKEN,
                        isColumnNameInsLowerCase);
                if (refreshToken != null) {
                    log.info("LOG PATCH: TABLE NAME: " + context.getPipelineConfiguration().getTableName() +
                            " ITERATION: " + context.getProperty("iteration") + " retrieving refresh token.");
                }
                TokenInfo tokenInfo = new TokenInfo(accessToken, refreshToken);
                log.info("LOG PATCH: TABLE NAME: " + context.getPipelineConfiguration().getTableName() +
                        " ITERATION: " + context.getProperty("iteration") + " token encryption is enabled : " +
